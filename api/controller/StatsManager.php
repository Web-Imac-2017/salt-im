...
class StatsManager {
  ...
  public function cancelVote($id, $name)
  {
    if ($this->hasVoted($id, $name)) {
      ...
      $q3->execute();
      $q4->execute();
      echo "ok le vote a été supprimé";
    }
    else {
      echo "vous n'avez pas voté et ne pouvez pas annuler votre vote";
      return false; // l'user n'a pas voté sur cette stat
    }
  }
